...
def _dist(pos1, pos2):#uses an underscore to make function private
...
class Shark:
    def __init__(self, pos, image):
        ...
        self.image = image
    def __repr__(self):
        return "Shark({}, {})".format(self.pos, self.image)
    def getPos(self):
        return self.pos
    def setPos(self, pos):
        self.pos = pos
    ...
    def getValidMoves(self):
        ...
        for i in valid_moves:
            if(1 <= i.getX() <= 20 and 1 <= i.getY() <= 20 and i != self.pos):
                valid_moves_2.append(i)
        return valid_moves_2
    def move(self, fish_positions):#something is broken somewhere here
        valid_moves = self.getValidMoves()
        move_scores = []
        for i in valid_moves:
            move_scores.append({"score": _dist(i, self.getPos()), "move": i})
